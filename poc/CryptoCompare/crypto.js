...
// OBJECT CONSTRUCTORS ................................................
function cryptoCoin(id, name, symbol) {
  this.id = id;
  this.name = name;
  this.symbol = symbol;
  this.priceUSD = '';
  ...
  fetch(url)
    ...
    .then(function (data) {
      ...
      dataObj.forEach(element => {
        let coinObj = new cryptoCoin(element.CoinInfo.Id, element.CoinInfo.FullName, element.CoinInfo.Name);
    ...
    .then(function (data) {
      ...
      for (let i = 0; i < coinArr.length; i++) {
        ...
        if (coin.Symbol.startsWith(coinSymbol)) {
          // create coin object
          let coinObj = new cryptoCoin(coin.Id.Id, coin.FullName, coin.Symbol);
...
function displayCoin(cryptoCoin) {
  ...
  tblCellAddBtn.innerHTML = `
      <button class="btn btn-primary rowBtn" data-toggle="tooltip" 
             title="Add" 
             onclick="addCoinToPortfolio('${cryptoCoin.symbol}')">
...
function addCoinToPortfolio(coinSymbol) {

  console.log("add to portfolio: " + coinSymbol);
